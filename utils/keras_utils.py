...
def multi_label_log_loss(y_pred, y_true):
    return tf.compat.v1.keras.backend.sum(tf.compat.v1.keras.backend.binary_crossentropy(y_pred, y_true), axis=-1)
...
def _average_gradient_norm(model, X_train, y_train, batch_size):
    ...
    grad_fct = tf.compat.v1.keras.backend.function(inputs=input_tensors, outputs=get_gradients)
...
def train_model(model:Model, dataset_id, dataset_prefix, dataset_fold_id=None, epochs=50, batch_size=128, val_subset=None,
    ...
    class_weight_dict = {}
    class_weight_dict = dict(zip(classes.astype(int), class_weight))
    
    uniquex, countsx = np.unique(y_train, return_counts=True)
    
    print("Number of training instances for each class:")

    print(np.asarray((uniquex, countsx)).T)
    
#     from sklearn.utils import class_weight as cwsk
#     class_weights_dict2 = dict(zip(np.unique(y_train), cwsk.compute_class_weight('balanced', np.unique(y_train), 
#                 y_train))) 

#    print("Class weights : ", class_weight)
    print("Class weights : ", class_weight_dict)
    #print("Class weights_dict seco: ", class_weight_dict2)
    ...
    model.fit(X_train, y_train, batch_size=batch_size, epochs=epochs, callbacks=callback_list,
              class_weight=class_weight_dict, verbose=2, validation_data=(X_test, y_test))
...
def f1_score(y_true, y_pred):
    def recall(y_true, y_pred):
        ...
        """
        true_positives = tf.compat.v1.keras.backend.sum(tf.math.round(tf.clip_by_value(y_true * y_pred, clip_value_min = 0, clip_value_max = 1)))
        possible_positives = tf.compat.v1.keras.backend.sum(tf.math.round(tf.clip_by_value(y_true, clip_value_min = 0, clip_value_max = 1)))
    ...
    def precision(y_true, y_pred):
        ...
        """
        true_positives = tf.compat.v1.keras.backend.sum(tf.math.round(tf.clip_by_value(y_true * y_pred, clip_value_min = 0, clip_value_max = 1)))
        predicted_positives = tf.compat.v1.keras.backend.sum(tf.math.round(tf.clip_by_value(y_pred, clip_value_min = 0, clip_value_max = 1)))
